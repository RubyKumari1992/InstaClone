...
extension FirebaseHelper {
    ...
    func updateProfileForPost(url: URL?, completion: @escaping (Bool?) -> ()) {
        var dataArray = [Posts]()
        let firestoredatabase = Firestore.firestore()
        let data = [ImageType.profile.rawValue: url?.absoluteString as Any] as [String: Any]
        self.getDataFromFirestore { (snapshot, result) in
            ...
            for values in dataArray {
                if values.postedBy == Auth.auth().currentUser?.email  && values.profileImage != url?.absoluteString {
